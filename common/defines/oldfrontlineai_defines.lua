...
--	ARMY_IMPORTANCE_FACTOR = 10.0,					-- Army factor for AI and calculations
--	VICTORY_POINTS_IMPORTANCE_FACTOR = 2.5,			-- State victory points importance factor for AI and calculations
...
--NMilitary = {
--	NDefines.NMilitary.COMBAT_VALUE_ORG_IMPORTANCE = 3		-- Multiplier on TotalOrganisation when determining the combat value of a division
...
--	DIPLOMACY_IMPROVE_RELATION_PP_FACTOR = 0.5,	-- Desire to boost relations adds total PP multiplied by this
...
--	NDefines.NAI.PLAN_MOVE_MIN_ORG_TO_ENEMY_PROVINCE = 10.0	-- Minimum organization a unit must have to consider moving into an enemy province (not attack move)
...
--	RELATIVE_STRENGTH_TO_INVADE_DEFENSIVE = 0.08,-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend, but while being a defensive country.
...
--	IRRATIONALITY_LAMBDA = 100,					-- Lambda given to Poisson Random function determining if a leader should act a bit irrational 											
...
--	NDefines.NAI.RELUCTANCE_TO_CHANGE_FRONT_FACTOR = 0.0			-- Factor for how reluctant the AI is to change a units order group.
