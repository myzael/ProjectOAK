...
NProduction = {
	...
	MIN_NAVAL_EQUIPMENT_CONVERSION_IC_COST_FACTOR = 0.02,		-- Minimum fraction of an equipment type's base industry capacity cost to use when converting a naval equipment, such as through ship refitting.
...
NMilitary = {
	...
	ARMY_FUEL_COST_MULT = 0.45,						-- fuel cost multiplier for all army related stuff
...
NAir = {
	...
	NAVAL_STRIKE_CARRIER_MULTIPLIER = 5.5,              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)																																													 
	...
	AIR_WING_COUNTRY_XP_FROM_TRAINING_FACTOR = 0.0075, 					--Factor on country Air XP gained from wing training
...
NNavy = {
	...
	NAVAL_MINES_IN_REGION_MAX = 500.0,								-- Max number of mines that can be layed by the ships. The value should be hidden from the user, as we present % so it's an abstract value that should be used for balancing.
	...
	NAVAL_MINES_DECAY_AT_PEACE_TIME = 2.00,							-- How fast mines are decaying in peace time. Planting mines in peace time may be exploitable, so it's blocked atm. That's why after war we should decay them too.
	...
	NAVAL_MINES_ACCIDENT_CRITICAL_HIT_CHANCES = 0.10,				-- If an accident happens, how likely it is to be a critical hit (caused by naval mines)
	...
	MAX_POSITIONING_PENALTY_FROM_HIGHER_SHIP_RATIO					= 0.25,  -- maximum penalty to get from larger fleets
	...
	SUBMARINE_BASE_TORPEDO_REVEAL_CHANCE 							= 0.20,		-- Chance of a submarine being revealed when it fires. 1.0 is 100%. this chance is then multiplied with modifier created by comparing firer's visibiility and target's detection
	...
	NAVAL_COMBAT_AIR_STRIKE_TARGETING_TO_SUB_DETECTION = 1.0,				-- Same, but for strike targeting (aka naval targeting)
