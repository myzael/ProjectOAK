...
NGame = {
	...
	LAG_DAYS_FOR_PAUSE = 100,						-- Days of client lag for pause of gamespeed.
	...
	NAVAL_COMBAT_CONVOY_SIZE = 0.16,				-- Size of a convoy in a naval battle. Used to determine the size of the fleet when intercepting convoy.
},
...
NDiplomacy = {
	...
	TENSION_VOLUNTEER_FORCE_DIVISION = 0.3,			-- Amount of tension generated for each sent division
...
NCountry = {
	EVENT_PROCESS_OFFSET = 15,						-- Events are checked every X day per character or province (1 is ideal, but CPU heavy)
	...
	SUPPLY_FROM_DAMAGED_INFRA = 0.4,                -- damaged infrastructure counts as this in supply calcs
...
NTechnology = {
	...
	BASE_YEAR_AHEAD_PENALTY_FACTOR = 10,		-- 3 Base year ahead penalty
...
NMilitary = {
	...
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.14,		   -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	BASE_CHANCE_TO_AVOID_HIT = 87,                 -- Base chance to avoid hit if defences left.
	...
	BASE_LEADER_TRAIT_GAIN_XP = 1.12,				   -- Base xp gain for traits per hour for armies.
	...
	ENCIRCLED_PENALTY = -0.35,                      -- penalty when completely encircled
	...
	UNIT_EXPERIENCE_PER_TRAINING_DAY = 0.003,
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 0.7,	 	       -- was 0.6--% of equipment lost to strength ratio in combat, so some % is returned if below 1
	...
	COMMANDER_LEVEL_UP_STAT_WEIGHTS = {5, 5, 3, 5}, -- level up stat random base weights attack, defense, planning, logistics
...
NAir = {
	...
	DETECT_EFFICIENCY_BASE = 0.2,						-- Base value for detection efficiency (once something detected, efficiency says how many airplanes was detected).
	DETECT_EFFICIENCY_FROM_RADAR = 0.5,					-- How much radars affect the efficiency.
	...
	AIR_NAVAL_KAMIKAZE_DAMAGE_MULT = 10.0,				-- Balancing value to increase usual damage to Strength for Kamikaze
	...
	ANTI_AIR_PLANE_DAMAGE_FACTOR = 0.75,					-- Anti Air Gun Damage factor
	ANTI_AIR_PLANE_DAMAGE_CHANCE = 0.35,					-- Anti Air Gun hit chance
...
NNavy = {
	...
	DETECTION_SUBS_CHANCE_BALANCE = 26.0,						-- Same as above, but for sub detection. It should be very hard to detect just the subs.
	...
	COMBAT_MIN_WIN_CHANCE_TO_KEEP_FIGHTING = 0.3,		 			-- DNM - original 0.2 - being at 60 per cent of the enemy's strength would usually lead to one side disengaging, and seems to work reasonably in tests -- compare our forces with enemy forces. 1.0 is equal win chances. Less then 1.0 is we have low chances. If we have at least X chances, then ships will attept to attack without waiting for the other ships that are on the way.
	...
	COMBAT_EVASION_TO_HIT_CHANCE_TORPEDO_MULT = 30.0,				-- the above evasion hit chance is multiplied by 150% if shooting with torpedos. Torpedoes are slow, so evasion matters more.
	...
	COMBAT_ESCAPING_SPEED_BALANCE = 0.5,							-- DNM, original 0.8, should make it a little easier for disengaging ships to escape -- Same as above, but used to balance when escaping.
	...
	COMBAT_MAX_DISTANCE_TO_ARRIVE = 90,							-- Max distance to arrive. When ships are on their way, their distance will never exceed this value.
	...
	COMBAT_RETREAT_DECISION_CHANCE = 0.04, 							-- There is also random factor in deciding if we should retreat or not. That causes a delay in taking decision, that sooner or later will be picked. It's needed so damaged fast ships won't troll the combat.
	...
	REPAIR_AND_RETURN_PRIO_LOW_COMBAT = 0.35,						-- DNM, original 0.1, BICE had 0.25 -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_PRIO_MEDIUM_COMBAT = 0.5,						-- DNM, original 0.5 -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_PRIO_HIGH_COMBAT = 0.6,						-- DNM, original 0.6 -- % of total Strength. When below, navy will go to home base to repair (in combat).
	REPAIR_AND_RETURN_AMOUNT_SHIPS_LOW = 0.3,						-- % of total damaged ships, that will be sent for repair-and-return in one call.
	...
	REPAIR_AND_RETURN_UNIT_DYING_STR = 0.25,							-- Str below this point is considering a single ship "dying", and a high priority to send to repair.
...
NAI = {
	...
	DIPLOMACY_ACCEPT_ATTACHE_OPINION_TRASHHOLD = -40, -- Value of opinion that will remove accepting penalty for receiveing the attache
	...
	TRADEABLE_FACTORIES_FRACTION = 2,			-- Will at most trade away this fraction of factories.
	...
	RELATIVE_STRENGTH_TO_INVADE = 0.8,			-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend.
	...
	ENTRENCHMENT_WEIGHT = 3.0,					-- AI should favour units with less entrenchment when assigning units around.
	...
	UPGRADE_DIVISION_RELUCTANCE = 5,					-- How often to consider upgrading to new templates for units in the field
	...
	LAND_COMBAT_FIGHTERS_PER_PLANE = 1.7,				-- Amount of air superiority planes requested per enemy plane
	...
	UPGRADES_DEFICIT_LIMIT_DAYS = 150,                           -- Ai will avoid upgrading units in the field to new templates if it takes longer than this to fullfill their equipment need
