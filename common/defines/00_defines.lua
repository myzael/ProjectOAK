...
NCountry = {
	...
	MAJOR_MIN_FACTORIES = 80,						-- need at least these many factories to become a major
    ...
    MAX_INTELLIGENCE_MANPOWER_DATA_DEVIATION = 0.5,	-- Max deviation in estimating enemy total manpower amount ( 0.0 - 1.0 )
	...
	SPECIAL_FORCES_CAP_BASE = 0.06,					-- Max ammount of special forces battalions is total number of non-special forces battalions multiplied by this and modified by a country modifier
	SPECIAL_FORCES_CAP_MIN = 36,					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
...
NProduction = {
	...
	MIN_FIELD_TO_TRAINING_MANPOWER_RATIO = 0.7,	-- Ratio which % of army in field can be trained
...
NPolitics = {
	...
	INSTANT_WIN_POPULARITY_WIN = 55, -- New party popularity
},
...
NMilitary = {
	...
	RECON_SKILL_IMPACT = 6, -- how many skillpoints is a recon advantage worth when picking a tactic.
	...
	LAND_COMBAT_STR_DAMAGE_MODIFIER = 0.08,        -- global damage modifier... but some equipment is returned at end of battles see : EQUIPMENT_COMBAT_LOSS_FACTOR
	...
	ADDITIONAL_COMBAT_WIDTH = 60,                  -- more opened up by support attack
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 0.5,	 	       -- was 0.6--% of equipment lost to strength ratio in combat, so some % is returned if below 1
	...
	FIELD_MARSHAL_XP_RATIO = 0.7,					-- xp gain ratio for army group leaders
...
NAir = {
	...
	NAVAL_STRIKE_DAMAGE_TO_STR = 1.5,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
	...
	ANTI_AIR_PLANE_DAMAGE_FACTOR = 1.0,					-- Anti Air Gun Damage factor
...
NNavy = {
	...
	DETECTION_CHANCE_MULT_FORMATION_SPREAD_CONVOY_RAIDING = 2,		
	...
	DETECTION_SUBS_CHANCE_BALANCE = 15.0,						-- Same as above, but for sub detection. It should be very hard to detect just the subs.
	...
	COMBAT_TORPEDO_ATTACK_USE_CHANCE = 0.4,						    -- chance of using torpedo attack (to avoid torpedo spam when at close distance)
	COMBAT_CLOSE_DISTANCE_MAX_HIT_CHANCE_BONUS = 0.4,				-- max bonus to hit chance when target is near. So if base is 0.5, dist.bonus is 0.1, and we are facing enemy at distance 0, the total hit chance is 0.6 (60%). When enemy is barely at our max shooting range, the bonus is 0.
	...
	COMBAT_ESCAPING_SPEED_BALANCE = 0.65,							-- Same as above, but used to balance when escaping.
	...
	BASE_CARRIER_SORTIE_EFFICIENCY = 0.55,							-- factor of planes that can sortie by default from a carrier
...
NAI = {
	...
	DIVISION_CREATE_MIN_XP = 999,				-- Minimum XP before attempting to create a fresh new division template.
	VARIANT_UPGRADE_MIN_XP = 999,				-- Minimum XP before attempting to create a new variant.
	UPGRADE_XP_RUSH_UPDATE = 5,				-- If XP is above this on the daily tick the AI will attempt to spend it
	...
	START_TRAINING_EQUIPMENT_LEVEL = 0.6,               -- ai will not start to train if equipment drops below this level
	...
	FALLBACK_LOSING_FACTOR = 0,							-- The lower this number, the longer the AI will hold the line before sending them to the fallback line
