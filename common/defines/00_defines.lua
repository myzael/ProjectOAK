...
NDiplomacy = {
	...
	NOT_READY_FOR_WAR_BASE = -500,					-- AI should be unwilling to enter accept a call to war if not ready for war against the relevant enemies.
...
NMilitary = {
	...
	IS_AMPHIBIOUS_LIMNIT = 0.1,					   -- If the amphibious movement is _above_ this limit it is considered amphibious
...
NNavy = {
	...
	COMBAT_ESCAPING_SPEED_BALANCE = 0.65,							-- DNM, original 0.8, should make it a little easier for disengaging ships to escape -- Same as above, but used to balance when escaping.
	...
	COMBAT_LEAD_TOO_MANY_SHIPS_PENALTY_SPEED = 0.18,					-- Speed penalty in % for ships coming into the battle (Those in the "incoming" field). Fe. 0.1 makes the ordinary speed drop down to 10% of the original speed when penalty is at max. Value should be between 0.0 and 1.0
	...
	AMPHIBIOUS_LANDING_PENALTY = -0.8,								-- amphibious landing penalty
	...
	NAVY_VISIBILITY_BONUS_ON_RETURN_FOR_REPAIR = 1,				-- Multiplier for the surface/sub visiblity when the heavily damaged fleet is returning to the home base for reparation. 1.0 = no bonus. 0.0 = invisible.
	...
	ENEMY_AIR_SUPERIORITY_IMPACT = -0.35,           					-- effect on ship efficiency due to enemy air superiorty
...
NAI = {
	...
	PLAN_MOVE_MIN_ORG_TO_ENEMY_PROVINCE = 25.0,	-- Minimum organization a unit must have to consider moving into an enemy province (not attack move)
	...
	EASY_TARGET_FRONT_IMPORTANCE = 20.0,			-- How much extra focus the AI should put on who it considers to be the easiest target.
	...
	MICRO_POCKET_SIZE = 6,						-- Pockets with a size equal to or lower than this will be mocroed by the AI, for efficiency.
	POCKET_DISTANCE_MAX = 15000,				-- shortest square distance we bother about chasing pockets
	...
	ENTRENCHMENT_WEIGHT = 0,					-- AI should favour units with less entrenchment when assigning units around.
	...
	STR_UNIT_WEAK = 0.45,						-- Strength (equipment) % for unit to be considered weak
	...
	AIR_WING_REINFORCEMENT_LIMIT = 400,
